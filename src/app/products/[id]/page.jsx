    ...
    <div className="max-w-6xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="flex flex-col">
        ...
        <div className="flex items-center mb-2">
          ...
          <span className="text-gray-700 text-sm">{product.ratings} Ratings</span>
        </div>

        <div className="flex items-baseline gap-2 mb-2">
          <span className="text-3xl font-bold text-red-600">৳{product.price}</span>
          {product.oldPrice && <span className="text-gray-700 line-through">৳{product.oldPrice}</span>}
          ...
          <div className="mb-4 text-gray-700">
            ...
            <div className="flex gap-2">
              {product.sizes.map((s, i) => (
                <span key={i} className="px-3 py-1 border rounded-lg cursor-pointer hover:bg-red-100">
                  {s}
                </span>
              ))}
            </div>
          ...
          <div className="mb-4 text-gray-700">
            ...
            <div className="flex gap-2">
              {product.colors.map((c, i) => (
                <span key={i} className="w-6 h-6 rounded-full border cursor-pointer" style={{ backgroundColor: c }} />
              ))}
            </div>
          </div>
        )}
